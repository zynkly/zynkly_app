...
// --- API CONFIGURATION ---
// Use your machine's LAN IP for physical devices (Make sure phone is on same Wi-Fi)
// Emulator will also work with this IP
const API_URL = 'http://192.168.1.35:3000/api/v1';
...
const api = {
  ...
  userInfo: (token) => api.request('/auth/me', 'GET', null, token),
  ...
  // Cart
  addToCart: (serviceId, token) => api.request('/cart/add', 'POST', { serviceId }, token),
  getCart: (token) => api.request('/cart', 'GET', null, token),
  updateCartItem: (itemId, quantity, token) => api.request(`/cart/update/${itemId}`, 'PUT', { quantity }, token),
  removeFromCart: (itemId, token) => api.request(`/cart/remove/${itemId}`, 'DELETE', null, token),
  ...
  // Orders & Payment
  createOrder: (token) => api.request('/orders/create-order', 'POST', {}, token),
  verifyPayment: (paymentData, token) => api.request('/orders/verify-payment', 'POST', paymentData, token),
};
...
const LoginScreen = ({ onLogin, onSignup, onGoogleLogin }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // In a real app, store response.data.token in AsyncStorage here
      // await AsyncStorage.setItem('token', response.data.token);
      onLogin(response.data.token); // Pass token up to parent
    } catch (error) {
...
const SignupScreen = ({ onLogin, onSignupSuccess }) => {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      onSignupSuccess(email); // Pass email for OTP screen
    } catch (error) {
...
const ServiceBookingScreen = ({ onBack, cartCount, onAdd, onCartPress }) => {
  ...
  const fetchServices = async () => {
    setLoading(true);
    try {
      const response = await api.getServices(selectedCategory);
  ...
  const renderServiceItem = ({ item }) => (
    <View style={styles.serviceListCard}>
      <Image source={{ uri: item.image || 'https://via.placeholder.com/150' }} style={styles.serviceListImage} />
      <View style={styles.serviceListContent}>
        <View style={styles.serviceListHeader}>
          <Text style={styles.serviceListTitle}>{item.name}</Text>
        ...
        <View style={styles.serviceListFooter}>
          <Text style={styles.serviceListDuration}>⏱ {item.duration} mins</Text>
...
const CartScreen = ({ cartData, onBack, onConfirm, onUpdateQuantity, onRemove }) => {
  const cartItems = cartData?.items || [];

  // Calculate totals
  const itemTotal = cartItems.reduce((sum, item) => sum + (item.price * item.quantity), 0);
  const taxes = itemTotal * 0.18; // 18% GST example
  const finalTotal = itemTotal + taxes;

  const renderCartItem = ({ item }) => (
    <View style={styles.cartItem}>
      <View style={{ flex: 1 }}>
        <Text style={styles.cartItemTitle}>{item.service.name}</Text>
        <Text style={styles.cartItemDuration}>⏱ {item.service.duration} mins</Text>
        ...
        ListEmptyComponent={
          <View style={{ alignItems: 'center', marginTop: 50 }}>
            ...
            <TouchableOpacity onPress={onBack} style={{ marginTop: 20 }}>
              <Text style={{ color: COLORS.primary, fontSize: 16 }}>Browse Services</Text>
            </TouchableOpacity>
          </View>
        }
      />
        ...
        <View style={styles.cartFooter}>
          <View style={styles.billDetails}>
            ...
            <View style={styles.billRow}>
              ...
              <Text style={styles.billValue}>₹{Math.round(taxes)}</Text>
            </View>
            <View style={[styles.billRow, styles.totalRow]}>
              <Text style={styles.totalLabel}>To Pay</Text>
              <Text style={styles.totalAmount}>₹{Math.round(finalTotal)}</Text>
            </View>
          </View>
...
const App = () => {
  ...
  const [pendingEmail, setPendingEmail] = useState(''); // For OTP
  const [userAddress, setUserAddress] = useState(null); // Store User Address
  const [isSignup, setIsSignup] = useState(false);
  const [isOtpVerification, setIsOtpVerification] = useState(false);
  const [showSplash, setShowSplash] = useState(false);
  const [isGoogleLogin, setIsGoogleLogin] = useState(false);
  const [currentScreen, setCurrentScreen] = useState('dashboard');
  const [cart, setCart] = useState(null); // Backend-synced cart
  ...
  const fetchCart = async () => {
    try {
      const res = await api.getCart(authToken);
      setCart(res.data);
    } catch (error) {
  ...
  const addToCart = async (service) => {
    if (authToken) {
      try {
        const res = await api.addToCart(service._id, authToken);
        setCart(res.data);
  ...
  const updateCartQuantity = async (itemId, quantity) => {
    try {
      const res = await api.updateCartItem(itemId, quantity, authToken);
      setCart(res.data);
    } catch (error) {
  ...
  const removeFromCart = async (itemId) => {
    try {
      const res = await api.removeFromCart(itemId, authToken);
      setCart(res.data);
    } catch (error) {
  ...
  if (currentScreen === 'payment') {
    const itemTotal = cart?.items?.reduce((sum, item) => sum + (item.price * item.quantity), 0) || 0;
    const taxes = itemTotal * 0.18;
    const totalAmount = Math.round(itemTotal + taxes);
      ...
      <PaymentScreen
        ...
        onPay={async (method) => {
          if (method !== 'upi' && method !== 'card') {
            Alert.alert('Info', 'Only UPI/Card supported in this demo integration');
            return;
          }

          try {
            // 1. Create Order
            const orderRes = await api.createOrder(authToken);
            const { orderId, amount, keyId } = orderRes.data;

            // 2. Open Razorpay
            const options = {
              description: 'Cleaning Service Booking',
              image: 'https://cdn-icons-png.flaticon.com/512/2917/2917995.png', // Cleaning icon
              currency: 'INR',
              key: keyId,
              amount: amount,
              name: 'Zynkly Services',
              order_id: orderId,
            ...
            console.error(error);
            Alert.alert('Initiation Failed', error.message || 'Could not start payment');
          }
        }}
      />
    );
